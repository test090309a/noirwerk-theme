...
(() => {
  ...
  let Doom = (() => {
    ...
    function reset() {
      ...
      RAW.forEach((row, y) =>
        [...row].forEach((ch, x) => {
          if (ch === "P")
            p = { x: x + 0.5, y: y + 0.5, a: 0, hp: 100, ammo: 24, score: 0 };
          if (ch === "E")
            enemies.push({
              x: x + 0.5,
              y: y + 0.5,
          ...
          if (ch === "h") pickups.push({ x: x + 0.5, y: y + 0.5, t: "hp" });
          if (ch === "a") pickups.push({ x: x + 0.5, y: y + 0.5, t: "am" });
        }),
      );
      msg = "";
      flashT = 0;
    ...
    function rayDist(ang) {
      const dx = Math.cos(ang),
        dy = Math.sin(ang);
      let d = 0;
      while (d < 20) {
        d += 0.05;
        if (solid(p.x + dx * d, p.y + dy * d)) break;
      }
      return d;
    }
    ...
    function blip(f0, f1, t, v, type = "square") {
      try {
        const c = ac(),
    ...
    const sfx = {
      shoot() {
        burst(0.09, 0.3);
        blip(430, 70, 0.09, 0.14);
      },
      hit() {
        blip(210, 60, 0.12, 0.2, "sawtooth");
      },
      boom() {
        burst(0.3, 0.3);
        blip(120, 28, 0.3, 0.2);
      ...
      hurt() {
        blip(140, 45, 0.25, 0.24, "sawtooth");
      },
      pick() {
        blip(660, 1320, 0.12, 0.14, "sine");
      },
    };
    ...
    function update(dt, now) {
      ...
      enemies.forEach((e) => {
        ...
        const dx = p.x - e.x,
          dy = p.y - e.y,
          d = Math.hypot(dx, dy);
        if (d < 7 && d > 0.6 && e.pain <= 0 && los(e.x, e.y, p.x, p.y)) {
          const s = 1.05 * dt,
            nx = e.x + (dx / d) * s,
            ny = e.y + (dy / d) * s;
    ...
    function render() {
      ...
      for (let x = 0; x < W; x++) {
        ...
        const fog = Math.min(0.82, perp / 11);
        if (fog > 0.02) {
      ...
      list.forEach((s) => {
        ...
        let img,
          ...
          alpha = 1;
        if (s.k) {
          img = s.k.t === "hp" ? sprHp : sprAmmo;
          sc = 0.34;
          off = 0.33;
        } else {
          const e = s.e;
          if (e.dead) {
            if (e.deathT > 0.5) return;
            alpha = 1 - e.deathT / 0.5;
            sc = 0.9 - e.deathT * 1.2;
            img = sprImp;
          } else img = e.pain > 0 ? sprHurt : sprImp;
        }
        const size = H / c.ty,
          hgt = size * sc,
          wdt = size * sc,
        ...
        const x0 = sx - wdt / 2;
        for (let i = 0; i < wdt; i++) {
          const cx = Math.floor(x0 + i);
          if (cx < 0 || cx >= W || zbuf[cx] <= c.ty) continue;
      ...
      if (flashT > 0) {
        ...
        gg.addColorStop(0.4, "rgba(214,0,28,.8)");
      ...
      enemies.forEach((e) => {
        if (!e.dead) {
          ctx.fillStyle = "rgba(214,0,28,.8)";
          ctx.fillRect(5 + e.x * 3, 5 + e.y * 3, 2, 2);
        }
      });
      if (dmgT > 0) {
        ctx.fillStyle = `rgba(214,0,28,${dmgT * 0.4})`;
      ...
      if (state === "dead" || state === "won") {
        ...
        ctx.fillText(
          state === "won" ? "SECTOR GESÄUBERT" : "SIGNAL VERLOREN",
          W / 2,
          H / 2 - 14,
        );
        ctx.fillStyle = "#fff";
    ...
    function boot() {
      ...
      const iv = setInterval(
        () => {
          if (li >= BOOT.length) {
            clearInterval(iv);
            setTimeout(startPlay, reduced ? 100 : 900);
            return;
          }
          const line = BOOT[li];
          if (ci < line.length) {
            if (line[ci] === "<") {
              const end = line.indexOf(">", ci);
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if ((state === "dead" || state === "won") && e.key === "Enter") {
        reset();
        state = "play";
        return;
      }
  ...
  if (fine) {
    ...
    qsa("[data-magnetic]").forEach((el) => {
      ...
      el.addEventListener("mouseleave", () => {
        if (!hasLib) return;
        gsap.to(el, { x: 0, y: 0, duration: 0.8, ease: "elastic.out(1,.4)" });
      });
    });
    ...
    btn.addEventListener("click", () => {
      ...
      qsa(".faq__item.open").forEach((o) => {
        if (o !== item) {
          ...
          if (hasLib) {
            gsap.to(o.querySelector(".faq__panel"), {
              height: 0,
              duration: 0.5,
              ease: "power3.inOut",
            });
  ...
  (function rain() {
    ...
    function build() {
      ...
      while (x < w) {
        ...
        const bh = h * (0.16 + Math.random() * 0.36);
    ...
    function tick(t) {
      ...
      grd.addColorStop(0.55, "#020202");
      grd.addColorStop(1, "#000");
      g.fillStyle = grd;
      g.fillRect(0, 0, w, h);

      const gx = w * 0.72 + Math.sin(t * 0.00012) * w * 0.06;
      const gy = h * 0.6;
      const rg = g.createRadialGradient(gx, gy, 0, gx, gy, w * 0.5);
      ...
      if (fl > 0) {
        g.fillStyle = `rgba(255,255,255,${(fl * 0.05).toFixed(3)})`;
        g.fillRect(0, 0, w, h);
      }
      ...
      for (const p of wins) {
        const tw = 0.5 + 0.5 * Math.sin(t * 0.0012 + p.p);
        g.fillStyle = p.r
          ? `rgba(214,0,28,${0.2 + 0.45 * tw})`
          : `rgba(255,255,255,${0.05 + 0.2 * tw})`;
  ...
  console.log(
    "%c NOIRWERK@DTF® ",
    "background:#D6001C;color:#000;font-weight:bold;padding:4px 8px;font-family:monospace",
  );
  console.log(
    "%cSECTOR 07 ACTIVE — tippe D·O·O·M, wenn du bereit bist.",
...
(function () {
  ...
  const overlayNum = document.getElementById("projectOverlayNum");
  const overlayTitle = document.getElementById("projectOverlayTitle");
  const overlayYear = document.getElementById("projectOverlayYear");
  const overlayCategory = document.getElementById("projectOverlayCategory");
  const overlayDesc = document.getElementById("projectOverlayDesc");
  const overlayTags = document.getElementById("projectOverlayTags");
  const body = document.body;

  // Sample project data (in a real WordPress site, this would come from the loop)
  const projects = [
    {
      id: "01",
      title: "Neon District",
      ...
      description:
        "Ein visuelles Identitätssystem für einen futuristischen Technologie-Hub. Von der Typografie bis zur Bewegtbildsprache – alles aus einer Hand.",
    ...
    },
    {
      ...
      description:
        "Ein modulares Designsystem für eine Next-Gen-Plattform. Fokus auf Performance, Barrierefreiheit und ästhetische Klarheit.",
      tags: ["UI/UX", "Designsystem", "SaaS"],
    },
    {
      id: "03",
      title: "Cyber Canvas",
      ...
      description:
        "Interaktive Echtzeit-Visualisierungen mit WebGL und Shadern. Eine immersive Erfahrung für Marken-Launches und Events.",
      tags: ["WebGL", "Shader", "Realtime"],
    },
  ];
  ...
  // Open overlay with project data
  function openOverlay(index) {
    const project = projects[index];
    if (!project) return;

    // Update content
    overlayNum.textContent = project.id;
  ...
  // 2. Click on overlay background (outside content)
  if (overlay) {
  ...
  // 4. TRIGGER: Click on any project card or element with data-project attribute
  // Option A: Click on elements with data-project-index
  ...
  // Option B: Click on work/project links (e.g., in hero CTA)
  document
    .querySelectorAll('.hero__cta a[href*="#projekte"], .panel__go')
    .forEach((el) => {
      el.addEventListener("click", function (e) {
        // Check if this link should open a specific project
        const parentPanel = this.closest(".post-panel");
        if (parentPanel) {
          e.preventDefault();
          // Find which project this panel represents
          const title = parentPanel.querySelector("h3 a")?.textContent || "";
          const index = projects.findIndex((p) => p.title === title);
          if (index >= 0) {
            openOverlay(index);
          } else {
            // Fallback: open first project
            openOverlay(0);
          }
        }
      });
  ...
  // 5. DEMO: Click on "Arbeit ansehen" button in hero
  const workBtn = document.querySelector('.hero__cta a[href*="#projekte"]');
  if (workBtn) {
    workBtn.addEventListener("click", function (e) {
      // If we want to open a project directly instead of scrolling
      // Uncomment the line below and remove the href
      // e.preventDefault();
      // openOverlay(0);
    });
  }

  // 6. DEMO: Click on project panels in the blog section
  document.querySelectorAll(".post-panel").forEach((panel, index) => {
    panel.addEventListener("click", function (e) {
      // Don't trigger if clicking on a link inside
      if (e.target.closest("a")) return;

      // Map panel to project (use index or title matching)
      const title = this.querySelector("h3 a")?.textContent || "";
      const projectIndex = projects.findIndex((p) => p.title === title);

      if (projectIndex >= 0) {
        e.preventDefault();
        openOverlay(projectIndex);
      } else if (index < projects.length) {
        // Fallback: use the panel index
        openOverlay(index % projects.length);
      }
    });
  });

  // Expose API globally for debugging or external use
  ...
  console.log(
    "✅ Project Overlay initialized. Click any project card to open.",
  );
  console.log("ℹ️ Use ProjectOverlay.open(index) in console to test.");
})();

// ============================================================
// PROJEKT STARTEN - STEP NAVIGATION (EINZIGE KORREKTE VERSION)
// ============================================================
(function() {
    "use strict";

    var form = document.getElementById("projectStartForm");
    if (!form) return;
    ...
    function updateStep(index) {
        for (var i = 0; i < steps.length; i++) {
            ...
            var inputs = steps[i].querySelectorAll("input, textarea, select");
            for (var j = 0; j < inputs.length; j++) {
                // ACHTUNG: data-required im HTML setzen!
                if (isActive && inputs[j].hasAttribute("data-required")) {
        ...
        for (var d = 0; d < dots.length; d++) {
            ...
            if (d === index) {
                ...
                dots[d].classList.add("done");
            }
        }

        for (var l = 0; l < lines.length; l++) {
            if (l < index) {
                lines[l].classList.add("done");
            } else {
                lines[l].classList.remove("done");
            }
        }
    ...
    for (var nb = 0; nb < nextBtns.length; nb++) {
        nextBtns[nb].addEventListener("click", function() {
            var currentStepEl = form.querySelector(".form-step.active");
            var inputs = currentStepEl.querySelectorAll("input, textarea, select");
            var valid = true;

            for (var i = 0; i < inputs.length; i++) {
                if (inputs[i].hasAttribute("required") && !inputs[i].value.trim()) {
                    valid = false;
                    inputs[i].style.borderColor = "#D6001C";
                    setTimeout(function(el) {
                        el.style.borderColor = "";
                    }.bind(null, inputs[i]), 2000);
                }
            }
    ...
    // Zurück-Buttons
    var prevBtns = form.querySelectorAll(".step-prev");
    for (var pb = 0; pb < prevBtns.length; pb++) {
        prevBtns[pb].addEventListener("click", function() {
    ...
    for (var r = 0; r < radios.length; r++) {
        radios[r].addEventListener("change", function() {
            ...
            if (step) {
                var next = parseInt(step.dataset.step, 10);
                if (next < steps.length) {
                    setTimeout(function() {
                        updateStep(next);
                    }, 400);
    ...
    form.addEventListener("keydown", function(e) {
        if (e.key === "Enter" && !e.target.closest("textarea")) {
            e.preventDefault();
            var activeStep = this.querySelector(".form-step.active");
            if (activeStep) {
                var stepNum = parseInt(activeStep.dataset.step, 10);
                if (stepNum === 4) {
                    // Submit
                    form.dispatchEvent(new Event("submit"));
                } else {
                    var nextBtn = activeStep.querySelector(".step-next");
                    if (nextBtn) {
                        nextBtn.click();
                    }
                }
    ...
    // Initial: Schritt 1 aktivieren
    updateStep(0);
})();


// ============================================================
// PROJEKT STARTEN – AJAX SUBMIT
// ============================================================
(function() {
    "use strict";

    var form = document.getElementById('projectStartForm');
    var overlay = document.getElementById('projectSuccessOverlay');
    ...
    form.addEventListener('submit', function(e) {
        ...
        formData.append('action', 'noirwerk_project_start_ajax');

        // AJAX-URL aus dem WordPress-Admin
        ...
        .then(function(data) {
            ...
            if (data.success) {
                ...
                var msg = data.data && data.data.message ? data.data.message : 'Bitte versuchen Sie es später erneut.';
                alert('Fehler: ' + msg);
            }
        })
